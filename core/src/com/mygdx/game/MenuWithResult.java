    ...
    public void render(float delta) {
        ...
        batch.begin();
            ...
            for (int i=0; i<strBestScore.length(); i++){
                ...
                number=new TextureRegion(numbers, n*25, 0, 25, 45);
                batch.draw(number, offsetX + i * 25, offsetY + offsetElementY * 3);
            }
            batch.draw(title, offsetX, offsetY+offsetElementY*8);
            batch.draw(textYourScore, offsetX, offsetY+offsetElementY*6);
            batch.draw(textBestScore, offsetX, offsetY+offsetElementY*4);
            batch.draw(btnRestart, offsetX, offsetY+offsetElementY*2);
            batch.draw(btnExitToMenu, offsetX, offsetY);
            //game.font.draw(batch, Integer.toString(width), 0, 0);
        batch.end();
    }
